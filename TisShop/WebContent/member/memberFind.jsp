...
<div class='container'>
<%
	String findType=request.getParameter("findType");			//검색유형
	String findKeyword= request.getParameter("findKeyword");	//검색어
...
<div class='col-md-10 col-md-offset-1 table-responsible'>
	...
	<h1 class='text-center text-primary' style="color:white;">회원 검색 결과 [검색어: <%=findKeyword %>]</h1>
...
<%
		...
		List<UserVO> arr=dao.findUsers(findType, findKeyword);
		//유효성체크
...
%>
			>
				...
				<td><%= (user.getM_state()==0)? "정회원":
						(user.getM_state()==-1)?"정지회원":"탈퇴회원" %></td>
				<td>
