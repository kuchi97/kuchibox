...
<script type="text/javascript">
	...
	var getUserInfo= function()
	{
		...
		var url="result2.jsp?phone="+phone;
	...
	var updatePage= function()
	{
		...
		{
			//성공적인 응답이 왔다면
			var res= req.responseXML;		//responseText: 응답유형이 text일때
											//responseXML: 응답유형이 xml일때
			
//			alert(res);		//object XMLDocument 로 응답
//			alert($(res).find("name").text());		//전화번호가 일치하면 이름을 반환
	
			var midx=$(res).find("idx").text();
			var mname=$(res).find("name").text();
			var maddr=$(res).find("addr").text();
			var mphone=$(res).find("phone").text();
